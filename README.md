...
**Open-source Solana DEX trading engine. Built for memecoin graduation strategies.**
...
Fathom is an event-driven trading engine purpose-built for Solana's memecoin lifecycle — specifically the moment a token graduates from pump.fun's bonding curve to an open-market DEX pool.

That transition creates a window. Liquidity arrives, price discovery begins, and the token either finds real demand or collapses. Most tools treat this as a notification event. Fathom treats it as a first-class data pipeline: discover → enrich → verify → decide → execute, with every stage instrumented and every decision auditable.

The engine is designed around a single principle: **the same strategy code runs in backtest, paper, and live with zero changes.** No rewriting logic for production. No separate research environment. The `GraduationSniper` strategy that backtests against historical graduation data is the same class instance that executes against real-time WebSocket feeds.
...
- **Graduation-native**: Not a generic DEX bot retrofitted for memecoins. Every component — data feeds, strategies, filters — is built around the pump.fun → PumpSwap/Raydium migration lifecycle.
...
- **Multi-source discovery**: DexScreener (profiles, boosts, search), pump.fun (graduated, top-runners), GeckoTerminal (trending, PumpSwap volume, transaction count) — 7 endpoints, 143+ mints per scan.
